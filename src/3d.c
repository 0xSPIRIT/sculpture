...
static void object_draw(Object3D *obj) {
    ...
    if (obj->state == OBJECT_DONE) {
        if (obj->timer == -1) {
            if (gs->should_update) narrator_tick();

            narrator_draw(RENDER_TARGET_MASTER, BLACK);
        ...
        for (int i = 0; i < 4; i++) {
            switch (obj->state) {
                ...
                case OBJECT_ROTY: case OBJECT_DONE: {
                    ...
                    obj->points[i].x = cos(t) * op[i].x;// - sin(t) * op[i].z;
                    obj->points[i].y = op[i].y;
                    obj->points[i].z = sin(t) * op[i].x;// + cos(t) * op[i].z;

                    // Then we're adding back the Z depth by
                    obj->points[i].y += obj->y;
                    obj->points[i].z += obj->z; // pushing it forward on the screen.
                    break;
                }
                case OBJECT_FALL: {
                    f32 ty = -obj->yrot;
                    ...
                    obj->points[i].x = z*cos(tx)*sin(ty) + y*sin(tx)*sin(ty) + x*cos(ty);
                    obj->points[i].y = y*cos(tx) - z*sin(tx);
                    obj->points[i].z = z*cos(tx)*cos(ty) - x*sin(ty) + y*sin(tx)*cos(ty);

                    obj->points[i].y += obj->y;
                    obj->points[i].z += obj->z; // Push it forward on the screen.
                    break;
                }
            }
        }
    ...
    project_square(vertices, obj->points);
