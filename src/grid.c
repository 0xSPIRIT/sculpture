...
static int number_neighbours_type(Cell *array, int type, int x, int y, int r) {
    int c = 0;
    for (int xx = -r; xx <= r; xx++) {
        for (int yy = -r; yy <= r; yy++) {
            if (xx == 0 && yy == 0) continue;
            if (!is_in_bounds(x+xx, y+yy)) continue;
            if (array[x+xx+(y+yy)*gs->gw].type == type) c++;
        }
    }
    return c;
}
...
static void grid_array_draw(int target, Cell *array, u8 alpha) {
    ...
    for (int y = 0; y < gs->gh; y++) {
        for (int x = 0; x < gs->gw; x++) {
            ...
            apply_lighting_to_color(&gs->lighting, &col, x, y);
