...
struct Text_Field text_field = {0};
...
void text_field_draw() {
    ...
    if (*text_field.description)
        description_surf = TTF_RenderText_Blended(font_consolas, text_field.description, (SDL_Color){180,180,180,255});
    if (*text_field.text)
        text_surf = TTF_RenderText_Blended(font_consolas, text_field.text, (SDL_Color){255,255,255,255});

    if (text_surf) {
        field_rect = (SDL_Rect){
            (gw*S)/2 - text_surf->w/2 - 16, GUI_H + (gh*S)/2 - text_surf->h/2 - 16,
            text_surf->w + 32, text_surf->h + 32
        };
        text_field_rect = (SDL_Rect){
            (gw*S)/2 - text_surf->w/2, GUI_H + (gh*S)/2 - text_surf->h/2,
    ...
    if (description_surf)
        description_texture = SDL_CreateTextureFromSurface(renderer, description_surf);
    if (text_surf)
    ...
    if (text_surf) {
        ...
        SDL_RenderCopy(renderer, text_texture, NULL, &text_field_rect);
    }
    ...
    if (description_surf) {
        ...
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_RenderFillRect(renderer, &description_rect);
        SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
